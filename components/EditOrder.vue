  ...
  >
    ...
    >
      ...
      <div v-if="isOrderEditable" class="text-center text-xs opacity-80 mt-1">
        Vuoi rimuovere uno snack o una bevanda dall'ordine? Non siamo avidi, ti
        restituiamo fino all'ultimo centesimo!
      </div>
...
const fetchDailyOrder = async (date: string) => {
  try {
    ...
    const response = await axios.get(
      `https://www.snacks2school.com/api/orders-by-day/`,
      {
...
const deleteOrderItem = async (
  ...
  try {
    ...
    const response = await axios.delete(
      `https://www.snacks2school.com/api/delete-order-item/${orderId}/${itemType}/${itemId}/?selected_date=${normalizedDate}`,
      {
        headers: {
          "X-CSRFToken": csrfToken,
          Authorization: `Token ${localStorage.getItem("authToken")}`,
